...
var nt = (p, g, O, L) => {
		if (g && typeof g == "object" || typeof g == "function")
			for (let M of kr(g)) !rt.call(p, M) && (O || M !== "default") && De(p, M, {
				get: () => g[M],
...
var ke = $e((qe, Ne) => {
	(function(p) {
		if (typeof qe == "object" && typeof Ne != "undefined") Ne.exports = p();
		...
		return function L(M, Q, C) {
			...
			4: [function(L, M, Q) {
				...
				function mr(e) {
					...
					if (e)
						for (var o in e) r[o] = typeof e[o] != "string" ? e[o].toString() : e[o];
					var i = new m(function(n, f) {
						try {
				...
				function Se() {
					for (var e = 1; e < arguments.length; e++) {
						var t = arguments[e];
						if (t)
							for (var r in t) t.hasOwnProperty(r) && (Xe(t[r]) ? arguments[0][r] = t[r].slice() : arguments[0][r] = t[r])
					}
					return arguments[0]
				}
				var jr = function() {
...
import {
	ViewController as ot,
	createDuplicateIframeWarning as it,
	createURL as tr,
	createModalNotReadyError as at
} from "@magic-sdk/provider";
var ft = {
	display: "none",
	position: "fixed",
...
var Te = class extends ot {
	...
	init() {
		this.test = "hello", this.iframe = new Promise(g => {
			let O = () => {
				if (st(encodeURIComponent(this.parameters))) it().log();
				else {
					let L = document.createElement("iframe");
					L.classList.add("magic-iframe"), L.dataset.magicIframeLabel = tr(this.endpoint).host, L.title = "Secure Modal", L.src = tr(`/send?params=${encodeURIComponent(this.parameters)}`, this.endpoint).href, ut(L), document.body.appendChild(L), g(L)
				}
			};
	...
	_post(g) {
		return se(this, null, function*() {
			...
			if (O && O.contentWindow) O.contentWindow.postMessage(g, this.endpoint);
			else throw at()
		})
	}
};
